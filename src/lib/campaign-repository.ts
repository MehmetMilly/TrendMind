...
function normalizeBrief(value: unknown): CampaignBrief {
  ...
  return {
    ...DEFAULT_BRIEF,
    campaignName: raw.campaignName?.trim() || DEFAULT_BRIEF.campaignName,
    brandName: raw.brandName?.trim() || "",
    productName: raw.productName?.trim() || "",
    audience: raw.audience?.trim() || "",
    goal: raw.goal?.trim() || "",
    platform: raw.platform?.trim() || DEFAULT_BRIEF.platform,
    language: raw.language?.trim() || DEFAULT_BRIEF.language,
    tone: raw.tone?.trim() || "",
